...
def handlePointsData(points_list, imu_data, imu_time_stamp, timeStartStamps, timeEndStamps):
    ...
    for k in range(points_len):
        ...
        count = cloudSize
        previous_timestamp_point = imuPointerScanLast
        point_time_idx = [[0,imuPointerScanLast-1]]
        ...
        for i in range(cloudSize):
            ...
            angle = np.arctan(point[2] / np.sqrt(point[1] * point[1] + point[0] * point[0])) * 180 / np.pi
            ...
            ori = -math.atan2(point[1], point[0]) - np.pi
            ...
            imu_PointerLast, _ = getLastAndCur(imu_time_stamp, point_time_stamp)
            if imu_PointerLast != previous_timestamp_point:
                previous_timestamp_point = imu_PointerLast
                point_time_idx.append([i, imu_PointerLast])
        #print(point_time_idx)
        point_time_idx = np.array(point_time_idx)
        
        for j in range(1, len(point_time_idx)):
            point_idx = point_time_idx[j, 0]
            imuPointerLast, imuPointerCur = point_time_idx[j-1, 1], point_time_idx[j-1, 1]+1
            ...
            delta_shift_between_measure_and_cal = np.array([[delta_shift_z_between_measure_and_cal,
                                                             ...
                                                             delta_shift_y_between_measure_and_cal, 0]])
            point_len = point_idx - point_time_idx[j-1, 0]
            
            delta_shift_between_measure_and_cal_list = np.repeat(delta_shift_between_measure_and_cal, point_len, axis=0)
            points[point_time_idx[j-1, 0]:point_idx] += delta_shift_between_measure_and_cal_list
